...
// ============================================================================
const Color colorPrimarioTitulo = Colors.teal; // Mantener verde para el título "NutriVision AI"
...
const TextStyle textoNormal = TextStyle(
  color: colorTexto,
  fontSize: 14,
);
...
class Receta {
  ...
  factory Receta.fromJson(Map<String, dynamic> json) {
    return Receta(
      ...
      ingredientes: (json['ingredientes'] as List<dynamic>?)?.map((e) => e.toString()).toList() ?? [],
...
final List<Receta> recetasDatabase = [
  // Copiado desde recetas copy; aquí mantengo los principales 17 ítems.
  Receta(
          id: '1',
      ...
      class _RecetasScreenState extends State<RecetasScreen> {
        ...
        List<String> getAllCategories() {
          final set = <String>{};
          for (var r in recetasDatabase) set.add(r.categoria);
          return ['Todos', ...set.toList()];
        }

        List<String> getAllTags() {
          final set = <String>{};
          for (var r in recetasDatabase) set.addAll(r.tags);
          return set.toList();
        }
        ...
        List<Receta> applyFilters() {
          ...
          if (selectedCategories.isNotEmpty) {
            list = list.where((r) => selectedCategories.contains(r.categoria)).toList();
          }

          if (selectedTags.isNotEmpty) {
            list = list.where((r) => r.tags.any((t) => selectedTags.contains(t))).toList();
          }
                        ...
                        child: pageItems.isEmpty
                            ? Center(
                                child: Column(
                                  ...
                                  children: const [
                                    Icon(Icons.restaurant, size: 72, color: colorTexto),
                                    SizedBox(height: 12),
                                    Text('No hay recetas con esos filtros', style: textoNormal),
                                  ],
                                ),
                              )
                            : ListView.builder(
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('Página $currentPage de $totalPages', style: textoSecundario),
                            const SizedBox(width: 8),
                            IconButton(
        ...
        Widget _tarjetaReceta(BuildContext context, Receta receta) {
          ...
          return GestureDetector(
            ...
            child: Card(
              ...
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14), side: const BorderSide(color: colorBorde, width: 1)),
              child: Padding(
                padding: const EdgeInsets.all(12),
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Center(
                        child: Text(receta.imagen, style: TextStyle(fontSize: imgFont)),
                      ),
                    ),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            ...
                            const SizedBox(width: 6),
                            Text('${receta.tiempoPreparacion} min', style: textoNormal),
                          ]),
                          const SizedBox(height: 8),
                          Wrap(spacing: 6, children: receta.tags.take(3).map((tag) => Chip(label: Text(tag, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600)), backgroundColor: colorTexto)).toList()),
                        ],
                      ),
                    ),
